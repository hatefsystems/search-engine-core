...
# Task 15.4: Troubleshooting Runbooks and Operational Procedures
...
- [ ] **Index Rebuild Procedures**
  ...
  for query in "machine learning" "آموزش برنامه‌نویسی" "深度学习"; do
  ...
  ## Success Criteria
  - [ ] Index document count matches expected (±2%)
...
- [ ] **Search Results Empty or Poor Quality**
  ...
  **Solution:**
  ...
  ## Escalation
  - If unresolved in 30 minutes → escalate to @tech-lead
  - If affecting multiple users → P0 incident, page on-call
  ```
...
- [ ] **Daily Health Checks**
  ...
  # 5. Smoke Test
  echo "\n--- Smoke Test ---"
  for query in "test" "آزمایش" "测试"; do
...
- **P0:** Service down, data loss, security breach → Page immediately
- **P1:** Major degradation, high latency, errors → Respond within 15 min
- **P2:** Minor issues, low impact → Resolve within business hours
- **P3:** Improvements, optimizations → Scheduled work
```
...
### Common Pitfalls
- ❌ Runbooks out of date with system changes
- ❌ Commands that don't work (copy-paste errors)
