...
# Task 15.3: Feature Glossary and Ranking Signal Documentation
...
- [ ] **BM25 Score (Lexical Relevance)**
  ...
  Document 1: Title="Machine Learning Tutorial", Body="Complete guide..."
    → BM25 ≈ 25.3 (exact title match + body occurrence)
  
  Document 2: Title="Introduction to AI", Body="...machine learning..."
    → BM25 ≈ 8.7 (partial body match only)
...
- [ ] **Embedding Similarity (Semantic Relevance)**
  ...
  - Training: Co-occurrence matrix → PPMI → Truncated SVD
  - Dimensions: 300
  ...
  Document 1: "TensorFlow and PyTorch comparison"
    → EmbSim ≈ 0.83 (high semantic similarity, different words)
  
  Document 2: "Deep learning frameworks overview"
    → EmbSim ≈ 0.91 (very high, overlapping terms + semantics)
  
  ### Tuning History
  - Initial: 200-dim embeddings, similarity ≈ 0.15 weight
  - Improved: 300-dim + PPMI preprocessing → 0.15 weight in FinalScore
...
- [ ] **HostRank (Domain Authority)**
  ...
  PageRank: PR(h) = (1-d)/N + d × Σ (PR(hi) / L(hi))
  ...
  - wikipedia.org → HostRank ≈ 0.95 (very high authority)
  - mit.edu → HostRank ≈ 0.88 (high authority .edu)
  - quantummagazine.com → HostRank ≈ 0.72 (established tech site)
  - personal-blog.com → HostRank ≈ 0.15 (low authority)
...
- [ ] **FinalScore Computation**
  ...
  FinalScore Calculation:
    = 0.55 × 22.5 + 0.15 × 0.82 + 0.10 × 0.88 + 0.06 × 0.35 
    + 0.05 × 1.3 + 0.04 × 0.95 + 0.03 × 0.85 - 0.08 × 0.02 + 0.04 × 0.85
    = 12.375 + 0.123 + 0.088 + 0.021 + 0.065 + 0.038 + 0.0255 - 0.0016 + 0.034
    = 12.768
  
  High score → Ranks near top of results
  
  ### Tuning History
  Version | Date | Changes | Impact
  v1.0 | 2024-01 | Initial weights from literature | Baseline
  v1.1 | 2024-02 | Increased EmbSim 0.10→0.15 | +12% NDCG@10
  v1.2 | 2024-03 | Added IntentAlign feature | +8% Nav success
  v1.3 | 2024-04 | Increased Spamness penalty 0.05→0.08 | -40% spam in top-10
...
- [ ] **Query Expansion & Synonym Handling**
  ...
  - Threshold and limits (≤3 expansions)
...
- [ ] **Stopword Filtering (Context-Aware)**
  ...
  - Multi-stage retrieval (filtered → original fallback)
...
- [ ] **Diversification (MMR)**
  ...
  - Domain repetition limits (≤3 results per domain in top-10)
  - Sub-intent coverage
...
docs/
├── features/
│   ├── README.md                    # Feature glossary overview
│   ├── lexical/
│   │   ├── bm25.md                  # BM25 detailed documentation
│   │   ├── anchor-match.md
│   │   └── url-quality.md
│   ├── semantic/
│   │   ├── embeddings.md            # Embedding similarity
│   │   ├── spell-correction.md
│   │   └── query-expansion.md
│   ├── authority/
│   │   ├── hostrank.md              # PageRank/authority
│   │   └── structured-boost.md
│   ├── quality/
│   │   ├── spam-detection.md
│   │   └── freshness.md
...
│   └── final-score.md               # FinalScore formula
```
...
### Common Pitfalls
- ❌ Vague descriptions without formulas
- ❌ Examples that don't match reality
- ❌ Missing range and typical value information
- ❌ No explanation of feature interactions
- ❌ Outdated documentation after tuning changes
