# 🎯 Task 08.3: Parameter Optimization & Auto-Tuning

## 📅 Sprint Info
- **Duration:** 4 days
- **Milestone:** M6 - Ranking Fusion
- **Priority:** P1
- **Depends On:** Task 08.1 (Feature Fusion) ✅, Task 02.8 (Offline Metrics Harness) ✅
- **Blocks:** Production deployment
- **Assignee:** TBD

> ⚠️ **Dependency Note:** This task originally referenced Task 10.1 (Proxy Metrics) but 10.1 is in M7 which runs *after* M6. Use the **Task 02.8 Offline Metrics Harness** (NDCG@10 from human judgment labels) as the objective function for weight optimization at this stage. Task 10.1's proxy metrics can be used for a second round of refinement in M7 once click data is available.

## 🎬 What You'll Build
Implement automated parameter tuning for ranking weights using proxy metrics. Use grid/line search to optimize weights on evaluation dataset, achieving ≥10% NDCG@10 improvement across all languages.
...
- [ ] NDCG@10 improvement ≥10% vs BM25-only baseline
- [ ] Optimal weights validated on holdout set
- [ ] Diversity maintained (≤3 per domain)
